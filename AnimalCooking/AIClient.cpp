...
AIClient::AIClient(Uint32 deltaTimePerOrder, Uint32 deltaTimeFirstOrder, TimerViewer* tv) : Component(ecs::AIClient),
	orMngr_(nullptr), availableOrders_(), initialOrders_(), allOrders_(false), group({ Resources::_FirstOfFoods_ + 1, Resources::Perrito3 })
{
	t = new Timer();
...
{
	return (allOrders_) ? 
		(Resources::FoodType)game_->getRandGen()->nextInt(group.first, group.second + 1)
		: 

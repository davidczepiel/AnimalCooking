	...
	if (deathTimer->isStarted()) {
		for (int i = 0; i < ingInfo.size(); i++) {
			if (ingInfo.at(i).ing != nullptr)
				...
				//Dependiendo de si el contador espec�fico de el ingrediente ha empezado o no renderizo la animacion de que va a morir o la de que est� muriendo
...
void IngredientsDeathAdversity::start()
{
	...
	while (ingInfo.size() < numKills) {
		...
		//Me quedo con la informaci�n del ingrediente escogido y me lo guardo en el vector 
		Ingredientinfo info;
	...
	ingInfo.at(0).ing->setInvincible(true);
	killsStarted = 1;
}
...
void IngredientsDeathAdversity::killIngredient()
{
	if (killsDone < numKills) {
		if (ingInfo.at(killsDone).ing != nullptr && std::find(multipleAdversityMngr_->getIngredientsPool()->getPool().begin(),
			multipleAdversityMngr_->getIngredientsPool()->getPool().end(), ingInfo.at(killsDone).ing) != multipleAdversityMngr_->getIngredientsPool()->getPool().end()) {
...
void IngredientsDeathAdversity::inspectIngredients()
{
	for (int i = 0; i < ingInfo.size(); i++) {
		...
		ingInfo.at(i).col %= 7;
		//Si el timer ya est� contando y ha sobrepasado la mitad de su tiempo este ingrediente ha muerto
...
void IngredientsDeathAdversity::startIngredient(int i)
{
	if (killsStarted < numKills) {
		if (killsStarted < ingInfo.size()) {
			...
			ingInfo.at(killsStarted).ing->setInvincible(true);
			killsStarted++;
		}
	}
}

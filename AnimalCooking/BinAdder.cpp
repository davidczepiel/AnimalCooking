...
BinAdder::BinAdder(EntityManager* em, jute::jValue& jsonLevel, jute::jValue& jsonGeneral, std::array<Entity*, 2>& player, const double casillaLength, const double offset) : em(em)
{
	...
	for (int i = 0; i < bins_.size(); ++i) {
		...
		bin->setPos(Vector2D(bins_[i]["pos"]["x"].as_double() * casillaLength + offset,
			bins_[i]["pos"]["y"].as_double() * casillaLength + offset));

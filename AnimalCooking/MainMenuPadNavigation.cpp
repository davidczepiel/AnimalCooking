...
void MainMenuPadNavigation::update() {
	//EN caso de que haya alg�n dispositivo conectado pregunto si ha pulsado algo para navegar
	//Si no hay ning�n mando conectado no me interesa hacer nada
	if (GPadController::instance()->playerControllerConnected(0) || GPadController::instance()->playerControllerConnected(1)) {
		...
		//Esta comprobaci�n y el bool est�n preparados para que si vuelves del menu de mapa al menu principal
...
bool MainMenuPadNavigation::dPadNotUsed() {
	//EN caso de que ningun player se est� intentando desplazar por el men�
	if (!GPadController::instance()->playerPressed(0, SDL_CONTROLLER_BUTTON_DPAD_LEFT) &&
...
void MainMenuPadNavigation::noArrowsFocused() {
	...
	//Tengo que volver a poner el bot�n central como que esta siendo seleccionado
	GETCMP2(selectButton, MenuButtonRenderer)->setFocused(true);

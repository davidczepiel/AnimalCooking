#include "Sink.h"
Sink::Sink(Vector2D pos,Transport* p1, Transport* p2, EntityManager* mng) :Entity(SDLGame::instance(),mng), Interactive(p1, p2,nullptr),cadence(100),canClean(true){
...
void Sink::action1(int iDp) {
		...
		if (SDL_GetTicks()-lastClean >canClean) {
			lastClean = SDL_GetTicks();
			canClean = false;
